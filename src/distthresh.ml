...
let invariant t =
  ...
  let farthest = List.fold_left (fun acc (d,_) -> max acc d) 0 list in
  let total = List.fold_left (fun acc (_,l) -> acc + List.length l) 0 list in
  let total_below_farthest = total - List.length (try Hashtbl.find t.tbl farthest with Not_found -> []) in
...
let throw_away_farthest t =
  ...
  | Some next_farthest ->
      ...
      let len_farthest = List.length (Hashtbl.find t.tbl t.farthest) in
      let len_next_farthest = List.length (Hashtbl.find t.tbl next_farthest) in
...
let to_list t = List.sort (fun (s1,_) (s2,_) -> compare s1 s2) & Hashtbl.to_list t.tbl
