...
module CMIDigest : sig
  ...
  let of_cu  cu  = try List.assoc cu.cu_name   cu.cu_imports     with _ -> assert false
  let of_ui  ui  = try List.assoc ui.ui_name   ui.ui_imports_cmi with _ -> assert false
  let of_cmi cmi = try List.assoc cmi.cmi_name cmi.cmi_crcs      with _ -> assert false
end
  ...
  | None ->
      ...
      let cmi_files =
        ...
        |> List.concat_map (fun modname -> 
          ...
          List.map (fun d -> d ^/ base) (FileDigest.find_by_base opam_build_table base))
      ...
      | cmis -> 
          ...
          | [cmt] -> [cmt]
          | cmts ->
              !!% "Warning: More than one cmt/cmti files for %s are found.@."
                (Module_path.to_string mpath);
              cmts
...
let rec get_packed opam_build_table path ml_path =
  ...
  | None ->
      ...
      [mpath, List.rev ml_path, None]

  | Some (_cmi, digest) ->
      (mpath, List.rev ml_path, Some digest) ::
      ...
      | cmt_paths ->
          ~~ List.concat_map cmt_paths ~f:(fun cmt_path ->
            ...
            | Interface _ -> []
            | Packed (_, paths) -> 
                let paths = List.map (fun p -> dir ^/ p) paths in
...
let load_link_unit opam_build_table resolver_in_ocamlfind_package filename = (* only for cma cmo cmx cmxa *)
    ...
    *)
      ...
      let find_mpath uname (* unit name like "List" *) ~digest =
        ...
        | None -> 
            ...
            | None -> 
                ...
                None
      in
      List.filter_map (fun (uname, digest) -> 
        let open Option in
        find_mpath uname ~digest >>= fun mpath -> return (mpath, digest)
      ) units
    in

    (* We cannot use Ppath since it introduce circular deps *)
    ...
    | Some (`CMO cu) -> 
        ...
        List.unique & get_packed opam_build_table filename & ppath filename
    | Some (`CMX ui) -> 
        [mpath, CMIDigest.of_ui ui],
        List.unique & get_packed opam_build_table filename & ppath filename
    | Some (`CMA lib) ->
        let mpath_md5s = List.unique & get_mpath_md5s (List.map (fun u -> u.Cmo_format.cu_name, CMIDigest.of_cu u) lib.Cmo_format.lib_units) in
        mpath_md5s,
        List.unique & List.concat_map (fun (mpath, _digest) -> 
          get_packed opam_build_table (Module_path.file ".cmi" mpath |> from_Some) 
            & pmpath mpath) mpath_md5s
    | Some (`CMXA lib) ->
        let mpath_md5s = List.unique & get_mpath_md5s (List.map (fun (u,_digest) -> u.Cmx_format.ui_name, CMIDigest.of_ui u) lib.Cmx_format.lib_units) in
        mpath_md5s,
        List.unique & List.concat_map (fun (mpath, _digest) -> 
          get_packed opam_build_table (Module_path.file ".cmi" mpath |> from_Some) 
            & pmpath mpath) mpath_md5s
...
let load_archive opam_build_table resolver_in_ocamlfind_package filenames =
    ...
    in
    let targets = List.unique & List.concat_map fst targets_and_tops_list in
    let reachables = List.unique & List.concat_map snd targets_and_tops_list in

    (* Check md5 uniqueness *)
    let tbl = Hashtbl.create 107 in
    reachables |> List.iter (fun (mpath, _ml_path, md5opt) ->
...
let cmi_without_value cmi_path =
  ...
  cmi_infos.cmi_sign |> List.for_all (function
    | Types.Sig_value _ 
    | Sig_module _
    | Sig_class _ -> false
    | Sig_type _
...
let module_file_path = Hashtbl.create 101

let find_cmi_file name =
  try Some (Misc.find_in_path_uncap !Config.load_path (name ^ ".cmi"))
  with Not_found -> None

let reset_cmi_file_cache () = Hashtbl.clear module_file_path
